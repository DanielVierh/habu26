...
export function createAppController(root: HTMLElement) {
  ...
  function formatEuroCentsForCsv(cents: number): string {
  ...
  function buildCsvLine(values: Array<string | number>): string {
    return values.map((value) => escapeCsvValue(String(value))).join(";");
  }

  function toMonthId(month: MonthNumber): string {
  ...
  function appendCsvBlock(
    ...
    rows.push(title);
    rows.push(buildCsvLine(headers));
    if (bodyRows.length === 0) {
      rows.push(buildCsvLine(["Keine Daten"]));
    } else {
      bodyRows.forEach((columns) => {
        rows.push(buildCsvLine(columns));
      });
    }
    rows.push("");
  }

  function getDetailedExportMonths(
    scope: "month" | "year",
  ): Array<{ year: number; month: MonthBook }> | null {
    if (!state.selectedYear) {
      showToast("Bitte zuerst ein Jahr auswählen.", "error");
      return null;
    }

    const selectedYearBook = state.years.find(
      (entry) => entry.year === state.selectedYear,
    );
    if (!selectedYearBook) {
      showToast("Ausgewähltes Jahr wurde nicht gefunden.", "error");
      return null;
    }

    if (scope === "month") {
      const month = selectedYearBook.months.find(
        (entry) => entry.month === state.selectedMonth,
      );
      if (!month) {
        showToast("Ausgewählter Monat wurde nicht gefunden.", "error");
        return null;
      }
      return [{ year: selectedYearBook.year, month }];
    }

    return selectedYearBook.months
      .slice()
      .sort((left, right) => left.month - right.month)
      .map((month) => ({ year: selectedYearBook.year, month }));
  }
  ...
  async function updateMonthlyBudgetWithPrompt(
    ...
    if (shouldApplyFuture) {
      ...
      await persistAllYears(`${baseReason} (inkl. zukünftiger Monate)`);
  ...
  async function importMiscExpensesFromCsv(file: File): Promise<void> {
    ...
    const duplicateKeys = new Set(
      month.miscCosts.map(
        (entry) => `${entry.description}__${entry.amountCents}`,
      ),
    );
  ...
  async function exportBackupCsv(): Promise<void> {
    const rows: string[] = [];
    rows.push(
      buildCsvLine([
        "Jahr",
    ...
    sortedYears.forEach((yearItem) => {
      yearItem.months
        ...
        .forEach((monthItem) => {
          ...
          const totalDiffCents = plannedBudgetCents - monthSummary.totalCents;

          const recordedIncomeCents = monthItem.incomes.reduce(
            (sum, entry) =>
              sum +
              (isRecordedIncomeSource(entry.incomeSource)
                ? entry.amountCents
          ...
          const monthFlow = incomeFlowByMonth.get(
            monthKey(yearItem.year, monthItem.month),
          );
          const carryoverFromPreviousMonth = monthFlow?.hasPreviousMonth
            ? formatEuroCentsForCsv(monthFlow.carriedFromPreviousCents)
            : "";
          ...
          rows.push(
            buildCsvLine([
              ...
              formatEuroCentsForCsv(foodBudgetCents),
              formatEuroCentsForCsv(monthSummary.foodCents),
              formatEuroCentsForCsv(foodBudgetCents - monthSummary.foodCents),
              formatEuroCentsForCsv(goingOutBudgetCents),
              formatEuroCentsForCsv(monthSummary.goingOutCents),
              formatEuroCentsForCsv(
                goingOutBudgetCents - monthSummary.goingOutCents,
              ),
              formatEuroCentsForCsv(fixedBudgetCents),
              formatEuroCentsForCsv(monthSummary.fixedCents),
              formatEuroCentsForCsv(fixedBudgetCents - monthSummary.fixedCents),
              formatEuroCentsForCsv(variableBudgetCents),
              formatEuroCentsForCsv(monthSummary.variableCents),
              formatEuroCentsForCsv(
                variableBudgetCents - monthSummary.variableCents,
              ),
              formatEuroCentsForCsv(miscBudgetCents),
              formatEuroCentsForCsv(monthSummary.miscCents),
              formatEuroCentsForCsv(miscBudgetCents - monthSummary.miscCents),
              formatEuroCentsForCsv(plannedBudgetCents),
              formatEuroCentsForCsv(monthSummary.totalCents),
              formatEuroCentsForCsv(totalDiffCents),
              formatEuroCentsForCsv(recordedIncomeCents),
    ...
    const url = URL.createObjectURL(blob);
    const backupFileName = `haushaltsbuch-backup-${getTodayIsoDate()}-${getBackupTimeId()}.csv`;
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = backupFileName;
    anchor.click();
    ...
    markBackupCompleted(backupFileName);
    render();
    showToast("CSV-Backup wurde exportiert.");
  }

  async function exportDetailedBackupCsv(
    scope: "month" | "year",
  ): Promise<void> {
    const exportMonths = getDetailedExportMonths(scope);
    if (!exportMonths || exportMonths.length === 0) {
      showToast(
    ...
    const firstExportMonth = exportMonths.at(0);
    if (!firstExportMonth) {
      showToast(
        "Für den gewählten Zeitraum sind keine Daten vorhanden.",
        "error",
    ...
    const rows: string[] = [];
    const selectedYear = firstExportMonth.year;
    const exportModeLabel = scope === "month" ? "Monat" : "Jahr";
    const firstMonth = firstExportMonth.month;

    rows.push(buildCsvLine(["Export", "Haushaltsbuch Detail CSV"]));
    rows.push(buildCsvLine(["Exportiert am", new Date().toISOString()]));
    rows.push(buildCsvLine(["Modus", exportModeLabel]));
    rows.push(buildCsvLine(["Jahr", selectedYear]));
    if (scope === "month") {
      rows.push(
        buildCsvLine([
          "Monat",
          `${monthLabel(firstMonth.month)} (${toMonthId(firstMonth.month)})`,
        ]),
      );
    }
    rows.push("");
    ...
    const fixedRows: Array<Array<string | number>> = [];
    const variablePositionRows: Array<Array<string | number>> = [];
    const variableCostRows: Array<Array<string | number>> = [];
    const miscRows: Array<Array<string | number>> = [];

    exportMonths.forEach(({ year, month }) => {
      month.days
        .slice()
        ...
        .forEach((dayEntry) => {
          dailyRows.push([
            ...
            toDateLabel(dayEntry.isoDate),
            formatEuroCentsForCsv(dayEntry.foodCents),
            formatEuroCentsForCsv(dayEntry.goingOutCents),
            formatEuroCentsForCsv(dayEntry.foodCents + dayEntry.goingOutCents),
          ]);
        });
      ...
      month.incomes
        ...
        .forEach((entry) => {
          incomeRows.push([
            ...
            entry.description,
            incomeSourceLabel(entry.incomeSource),
            formatEuroCentsForCsv(entry.amountCents),
          ]);
        });
      ...
      month.fixedCosts
        ...
        .forEach((entry) => {
          fixedRows.push([
            ...
            monthLabel(month.month),
            entry.name,
            formatEuroCentsForCsv(entry.plannedCents),
            formatEuroCentsForCsv(entry.actualCents),
            formatEuroCentsForCsv(entry.plannedCents - entry.actualCents),
          ]);
        });
      ...
      month.variablePositions
        ...
        .sort((left, right) => left.name.localeCompare(right.name, "de-DE"))
        .forEach((entry) => {
          variablePositionRows.push([
            year,
            monthLabel(month.month),
            entry.name,
            formatEuroCentsForCsv(entry.budgetCents),
            formatEuroCentsForCsv(entry.actualCents),
            formatEuroCentsForCsv(entry.budgetCents - entry.actualCents),
          ]);
        });
    ...
    appendCsvBlock(
      rows,
      "Tageswerte",
      ["Jahr", "Monat", "Datum", "Essen (€)", "Freizeit (€)", "Tag Summe (€)"],
      dailyRows,
    );
    appendCsvBlock(
      rows,
      "Einnahmen",
      ["Jahr", "Monat", "Datum", "Beschreibung", "Quelle", "Betrag (€)"],
      incomeRows,
    );
    appendCsvBlock(
      rows,
    ...
    appendCsvBlock(
      ...
      variablePositionRows,
    );
    appendCsvBlock(
      rows,
      "Variable Kosten",
      ["Jahr", "Monat", "Datum", "Beschreibung", "Betrag (€)"],
      variableCostRows,
    );
    appendCsvBlock(
      rows,
    ...
    const scopeIdentifier =
      scope === "month"
        ? `${selectedYear}-${toMonthId(firstMonth.month)}`
        : String(selectedYear);
    const backupFileName = `haushaltsbuch-backup-detail-${scope}-${scopeIdentifier}-${getBackupTimeId()}.csv`;
    ...
    showToast(
      scope === "month"
        ? "Detailliertes CSV-Backup für den Monat wurde exportiert."
        : "Detailliertes CSV-Backup für das Jahr wurde exportiert.",
    );
  }
  ...
  async function removeSavedKeywordEvaluation(id: string): Promise<void> {
    const toRemove = state.savedSearchEvaluations.find(
      (entry) => entry.id === id,
    );
    if (!toRemove) {
      return;
  ...
  function render(): void {
    ...
    const incomeMinusPlannedBudgetsCents =
      recordedIncomeTotalCents - monthPlannedBudgetTotalCents;
    ...
    const yearComparisonSums = {
      ...
      income: yearComparisonIncomeValues.reduce((sum, value) => sum + value, 0),
    ...
    const monthTotalBudgetStatusClass = budgetStatusClass(
      monthSummary.totalCents,
    ...
    const allYearsRows = sortedYears.map((yearItem) => {
      ...
      return {
        ...
        foodAndGoingOutCents: summary.foodCents + summary.goingOutCents,
                          ...
                          <div class="spark-bar" title="${monthLabel(row.month)}: ${centsToEuro(row.actualCostCents)}">
                            <div class="spark-bar-stack">
                              <div class="spark-bar-track" aria-hidden="true">
                                <div class="spark-bar-fill spark-bar-fill-layered spark-bar-fill-actual" style="height:${height}"><span class="spark-bar-fill-value">${centsToEuro(row.actualCostCents)} €</span></div>
                              </div>
                            </div>
              ...
              .map((row) => {
                ...
                return `
                          <div class="spark-bar" title="${monthLabel(row.month)}: ${centsToEuro(row.foodCents)}">
                            <div class="spark-bar-stack">
                              <div class="spark-bar-track" aria-hidden="true">
                                <div class="spark-bar-fill spark-bar-fill-layered spark-bar-fill-actual" style="height:${height}"><span class="spark-bar-fill-value">${centsToEuro(row.foodCents)} €</span></div>
                              </div>
                            </div>
                            <div class="spark-bar-label">${monthLabel(row.month).slice(0, 3)}</div>
                          </div>
              ...
              .map((row) => {
                ...
                return `
                          <div class="spark-bar" title="${monthLabel(row.month)}: ${centsToEuro(row.goingOutCents)}">
                            <div class="spark-bar-stack">
                              <div class="spark-bar-track" aria-hidden="true">
                                <div class="spark-bar-fill spark-bar-fill-layered spark-bar-fill-actual" style="height:${height}"><span class="spark-bar-fill-value">${centsToEuro(row.goingOutCents)} €</span></div>
                              </div>
                            </div>
                            <div class="spark-bar-label">${monthLabel(row.month).slice(0, 3)}</div>
                          </div>
              ...
              .map((row) => {
                ...
                return `
                          <div class="spark-bar" title="${row.year}: ${centsToEuro(row.actualTotalCents)}">
                            <div class="spark-bar-stack">
                              <div class="spark-bar-track" aria-hidden="true">
                                <div class="spark-bar-fill spark-bar-fill-layered spark-bar-fill-actual" style="height:${height}"><span class="spark-bar-fill-value">${centsToEuro(row.actualTotalCents)} €</span></div>
                              </div>
                            </div>
          ...
          <article class="card compact-month-overview" aria-label="Kompakte Monatsübersicht">
            ...
            <section class="compact-month-summary-row" aria-label="Monatssummen">
              <article class="compact-month-summary-box">
                ...
                <div class="compact-month-summary-amount">${centsToEuro(monthPlannedBudgetTotalCents)}</div>
              </article>
              <article class="compact-month-summary-box">
  ...
  function bindEvents(): void {
    ...
    const openEvaluationModalButton = root.querySelector<HTMLButtonElement>(
    ...
    const importMiscCsvButton =
      root.querySelector<HTMLButtonElement>("#import-misc-csv");
    ...
    const importFixedCsvButton =
      root.querySelector<HTMLButtonElement>("#import-fixed-csv");
    const importFixedCsvInput = root.querySelector<HTMLInputElement>(
      "#import-fixed-csv-input",
    );
    const clearFixedCostsButton =
      root.querySelector<HTMLButtonElement>("#clear-fixed-costs");
    ...
    const backupExportCsv =
      root.querySelector<HTMLButtonElement>("#backup-export-csv");
    const backupExportDetailedMonthCsv = root.querySelector<HTMLButtonElement>(
      "#backup-export-detailed-month-csv",
    );
    const backupExportDetailedYearCsv = root.querySelector<HTMLButtonElement>(
      "#backup-export-detailed-year-csv",
    );
    ...
    backupExportCsv?.addEventListener("click", async () => {
      try {
        await exportBackupCsv();
      } catch (error) {
        console.error("CSV-Backup-Export fehlgeschlagen", error);
        showToast("CSV-Backup konnte nicht exportiert werden.", "error");
      }
    });

    backupExportDetailedMonthCsv?.addEventListener("click", async () => {
      try {
        await exportDetailedBackupCsv("month");
      } catch (error) {
        console.error("Detaillierter Monats-CSV-Export fehlgeschlagen", error);
        showToast(
          "Detailliertes Monats-CSV konnte nicht exportiert werden.",
          "error",
        );
      }
    });

    backupExportDetailedYearCsv?.addEventListener("click", async () => {
      try {
        await exportDetailedBackupCsv("year");
      } catch (error) {
        console.error("Detaillierter Jahres-CSV-Export fehlgeschlagen", error);
        showToast(
          "Detailliertes Jahres-CSV konnte nicht exportiert werden.",
          "error",
        );
      }
    });
